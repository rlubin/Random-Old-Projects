...
const getPriceChangeVolumeLarger = async (tickers, percent, volume) => {
	...
	for (ticker of tickers) {
		...
		if (typeof data[0]['close'] === 'undefined') {
			console.log("ERROR: data[0]['close'] is undefined\n")
			continue
		}
		if (typeof data[1]['close'] === 'undefined') {
			console.log("ERROR: data[1]['close'] is undefined\n")
			continue
		}
		if (typeof data[0]['volume'] === 'undefined') {
			console.log("ERROR: data[0]['volume'] is undefined\n")
			continue
		}
		...
		count = count + 1
		console.log(
			`${count} of ${tickers.length}\n${todays_close} > ${
				yestersdays_close * (1.0 + percent * 0.01)
			} and ${todays_volume} > ${volume}`
		)
